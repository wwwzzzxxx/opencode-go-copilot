...
const zhCN: Record<string, string> = {
	...
	// extension.ts - manual model list refresh (opencodego.updateModelList)
	"Show Log": "查看日志",
	"request failed": "请求失败",
	"OpenCode Go model list updated successfully.": "OpenCode Go 模型列表已更新。",
	"Failed to update OpenCode Go model list. See output for details.": "更新 OpenCode Go 模型列表失败，详情见输出面板。",
	"Failed to update OpenCode Go model list: no model catalog available. See output for details.": "刷新失败：没有可用的模型目录，详情见输出面板。",
	"Could not fetch the online model catalog ({0}). Kept the previously fetched catalog ({1} models).": "无法获取线上模型目录（{0}）。已保留上次成功获取的目录（{1} 个模型）。",
	"Could not fetch the online model catalog ({0}). Using the built-in snapshot ({1} models, dated {2}) — recently added models may be missing.": "无法获取线上模型目录（{0}）。当前使用内置快照（{1} 个模型，快照日期 {2}），新加入的模型可能缺失。",
	"Catalog updated ({0} models). The server model list was not checked: no API key configured.": "模型目录已更新（{0} 个模型）。未检查服务端可用列表：未配置 API 密钥。",
	"Catalog updated ({0} models), but the server's model list could not be fetched ({1}). The list may include models the server does not serve.": "模型目录已更新（{0} 个模型），但无法获取服务端的模型列表（{1}）。列表可能包含服务端不提供的模型。",
	"The server model list could not be refreshed ({0}); the list is filtered by an earlier fetch ({1} models shown), so newly added models may be missing.": "服务端模型列表无法刷新（{0}），当前用更早一次获取的列表做过滤（显示 {1} 个模型），新加入的模型可能被隐藏。",
	"Model list updated: {0} models available. New: {1}": "模型列表已更新：可用 {0} 个模型。新增：{1}",
	"Model list updated: {0} models available, no new models.": "模型列表已更新：可用 {0} 个模型，本次无新增。",
	"Model list updated: {0} models available.": "模型列表已更新：可用 {0} 个模型。",
	" and {0} more": " 等共 {0} 个",
